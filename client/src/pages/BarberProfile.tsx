...
const BarberProfile: React.FC = () => {
  const { shopSlug, barberId } = useParams<{ shopSlug: string, barberId: string }>();
  ...
  if (!barber) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4 pt-32">
        ...
        <Link to={`/${shopSlug || ''}/barbers`} className="btn btn-outline">Back to Barbers</Link>
      </div>
    );
  }
  ...
  return (
    <div className="pt-[120px] pb-20 bg-bgPrimary min-h-screen">
      <div className="container mx-auto px-4 sm:px-8 max-w-[1000px]">
        <Link to={`/${shopSlug || ''}/barbers`} className="inline-flex items-center gap-2 text-gray-400 hover:text-accent transition-colors mb-8 text-[0.85rem] uppercase tracking-wider font-medium">
        ...
        <div className="bg-white/[0.02] border border-white/[0.06] rounded-2xl p-8 md:p-12 mb-12 flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-12 relative overflow-hidden">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <Link to={`/${shopSlug || ''}/book`} className="btn btn-accent px-10 py-3.5 shadow-[0_0_20px_rgba(212,175,55,0.2)]">
              Book Appointment
            </Link>
          </div>
        </div>
